...
mod_apka_server <- function(id) {
  ...
  output$mainPlot <- renderPlotly({
    ...
    dane %>% 
      ...
      mutate(Prizes, case_when(Prizes == 0 ~ NA,
                               TRUE ~ as.numeric(Prizes,scientific=FALSE))) %>% 
      ggplot(aes(x = Year, y = Prizes)) +
      geom_point(colour="#37A8E8") +
      geom_line(colour="#37A8E8") +
      scale_x_continuous(
        breaks = unique(dane$Year),         # tylko istniejące lata
        labels = as.character(unique(dane$Year))  # pokazuj jako liczby całkowite
      ) +
      scale_y_continuous(labels = function(x) format(x, scientific = FALSE)) +
      labs(title = paste0("Suma nagród w ",tournaments[i]),x = "Rok", y = "Suma nagród") +
      theme_minimal()
  })
  ...
  output$extraPlot <- renderPlotly({
    ...
    ggplotly(
      ggplot(dane2, aes(x = as.factor(Year), y = Peak_viewership)) +
        geom_point(colour="#37A8E8") +
        scale_y_continuous(labels = function(x) format(x, scientific = FALSE)) +
        labs(title="Szczytowa oglądalność w World Darts Championship",x = "Rok", y = "Oglądalność")+
        theme_minimal()
    )
  })
})
